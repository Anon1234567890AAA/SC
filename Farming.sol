...
contract Farming is Ownable {
    ...
    IERC20 public RBK;
    uint256 public RBKPerBlock = uint256(40 ether); //40 RBK
    ...
    function setTokensAddress(IERC20 _RBK, IERC20 _lpToken) external onlyOwner {
        require(
            address(RBK) == address(0) &&
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.number > liquidityMining.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 RBKReward = multiplier.mul(RBKPerBlock);
            accPerShare = liquidityMining.accPerShare.add(
                RBKReward.mul(1e12).div(lpSupply)
            );
        }
        return
    ...
    function safetokenTransfer(address to, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 RBKBalance = RBK.balanceOf(address(this));
        if (amount > RBKBalance) {
            RBK.safeTransfer(to, RBKBalance);
            return RBKBalance;
        } else {
            RBK.safeTransfer(to, amount);
            return amount;
        }
    }

    function settokenPerBlock(uint256 _RBKPerBlock) external onlyOwner {
        require(_RBKPerBlock > 0, "RBK per block should be greater than 0!");
        RBKPerBlock = _RBKPerBlock;
    }
}
